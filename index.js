	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Oiii sou o bot do sasuke seja@${num.split('@')[0]}\nBem vindo(a) ao grupo seje ativo(a)❤️🙂*${mdata.subject}*`
				...
				teks = `Xauzinho foi de ralo @${num.split('@')[0]}👋😂`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				...
				levelnol: '*Pqp kskst level* 0 Valeu ghost haha ',
				error: {
			...
			tels = body.slice(6)
               ...
               if (data.error) return reply(data.error)
				...
				case 'meme':
					...
					break
                case 'vtmnc':
		            if (args.length < 1) return reply ('me chingar e facil quero ver cume meu cu😡')
		            break
                case 'bot?':
		            if (args.length < 1) return reply ('Oiiee')
		            break
                case 'qual seu nome bot?':
		            if (args.length < 1) return reply ('Olá me chamo gilbertinho o bot de konoha')
		            break
                case 'tudo bem?':
		            if (args.length < 1) return reply ('tudo bem sim obrigado por perguntar')
		            break
                case 'Oii':
		            if (args.length < 1) return reply ('Oi linda passa o zap')
		            break
                case 'freefire':
		            if (args.length < 1) return reply ('1577508312 id do bot no ff')
		            break
                case 'animes.com':
		            if (args.length < 1) return reply ('https://nowanimes.com/')
		            break
                case 'wiki naruto':
		            if (args.length < 1) return reply ('https://naruto.fandom.com/pt-br/wiki/Wiki_Naruto')
		            break
                case 'Kkkskka':
		            if (args.length < 1) return reply ('kakakakjajajajakakak')
		            break
                case '😞':
		            if (args.length < 1) return reply ('F😞')
		            break
                case 'vagabundo': 
		            if (args.length < 1) return reply ('só o sasuke kakakakak😂')
		            break
                case 'fdp':
		            if (args.length < 1) return reply ('teu pai😡')
		            break
                case 'edson piadas':
		            if (args.length < 1) return reply ('piu...😂💩 zueira porque a velhinha não usa relógio?  porque ela e senhora😂')
		            break
                case 'linkputeiro':
		            if (args.length < 1) return reply ('https://chat.whatsapp.com/EqDtmoL1pwG8H23erY8f2t')
		            break
                case 'linkescritorio':
		            if (args.length < 1) return reply ('https://chat.whatsapp.com/EqDtmoL1pwG8H23erY8f2t')
		            break
                case 'linkvaledofim':
		            if (args.length < 1) return reply ('https://chat.whatsapp.com/B5DeIjF4zck4j1ZqbKKRKQ')
		            break
                case 'linkloja':
		            if (args.length < 1) return reply ('https://chat.whatsapp.com/KboyHhv7aaU1iEf5dIMfWs')
		            break
                case 'linkportão':
		            if (args.length < 1) return reply ('https://chat.whatsapp.com/C4gaxNJRmPlEfN0TiF5x3m')
		            break
				case 'hentai':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSrg0o6xAoNT6LOhzZo16X0x6svqm3ons0SeA&usqp=CAU`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'o cara que bate punheta pra desenho 2d😂'})
					break
				case 'Mia kalifa':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://pbs.twimg.com/media/Dy8uVEFWkAA_S3v.jpg`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'punheteiro do krl kkkkkkkkkkk😂'})
					break
			   case 'porno':
				    if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply(mess.only.admin)
